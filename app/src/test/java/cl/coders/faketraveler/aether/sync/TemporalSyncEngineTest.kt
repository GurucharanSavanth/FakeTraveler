...
class TemporalSyncEngineTest {
    ...
    fun setUp() {
        ...
        injector = RecordingInjector(locationManager)
    ...
    @Test
    fun `all three providers receive identical elapsedRealtimeNanos`() = runBlocking {
    ...
    @Test
    fun `GPS accuracy is less than NETWORK accuracy`() = runBlocking {
    ...
    @Test
    fun `extras contain satellites and magnetic_declination`() = runBlocking {
        val result = engine.injectLocation(
            latitude = 35.6762,
            longitude = 139.6503,
        )
    ...
    @Test
    fun `all providers receive correct coordinates`() = runBlocking {
        val lat = 51.5074
        val lon = -0.1278
        val alt = 42.0
        val result = engine.injectLocation(
    ...
    private fun lastLocation(provider: String): Location? =
        (injector as RecordingInjector).locations[provider]

    private class RecordingInjector(
        locationManager: LocationManager,
    ) : MultiProviderInjector(locationManager) {
        val locations = mutableMapOf<String, Location>()

        override fun inject(providerName: String, location: Location): Result<Unit> = runCatching {
            locations[providerName] = Location(location)
        }
    }

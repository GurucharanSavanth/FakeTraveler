...
import cl.coders.faketraveler.aether.data.db.ElevationCacheDao
import cl.coders.faketraveler.aether.data.db.ElevationCacheEntity
...
class ElevationEnricher(
    ...
    suspend fun evictStale(): Result<Unit> = runCatching {
        ...
        elevationCacheDao.evictOlderThan(cutoff.toEpochMilliseconds())
    }

    // ---- internal --------------------------------------------------------

    private fun isFresh(fetchedAt: Long, now: Instant): Boolean =
        (now - Instant.fromEpochMilliseconds(fetchedAt)) < TTL
